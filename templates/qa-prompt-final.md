...
• НЕЙТРАЛЬНЫЕ ФОРМУЛИРОВКИ: не «атака/эксплойт/инъекция/секрет-оружие» — пиши «корректная работа с
  БД / проверка входных значений / устойчивость к произвольным строкам» (иначе модель уходит в отказ).
...
Бери постановки тестов 1.1–7.4 из этого файла (секторы 1–7 выше).
